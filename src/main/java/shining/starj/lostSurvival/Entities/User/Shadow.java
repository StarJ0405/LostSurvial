...
public class Shadow extends CustomEntities {
    ...
    public static class CustomZombie extends Zombie {
        ...
        public void tick() {
            ...
            if (PlayerStore.getStore(owner).isDead() || !GameStore.getInstance().getStatus().equals(GameStatus.START)) {
                this.getBukkitEntity().remove();
                return;
            }
            ...
            if (GameStore.getInstance().getStatus().equals(GameStatus.START)) {
                ...
                if (this.tick == 30) {
                    ...
                    Vector dir = owner.getLocation().clone().add(0, 0.5d, 0).subtract(start).toVector().setY(0).normalize().multiply(0.5d);
                    if (projectile % 2 == 1) {
