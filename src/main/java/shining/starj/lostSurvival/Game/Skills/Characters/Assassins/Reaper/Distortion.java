...
public class Distortion extends AbstractSkill {
    ...
    public List<String> getLore(int level, boolean upgrade) {
        ...
        list.addFirst(ChatColor.RED + "쉬프트 상태일 때만 발동");
        if (level == 0) {
            list.addFirst(ChatColor.WHITE + "이동거리에 있던 대상에게 피해를 준다.");
            list.addFirst(ChatColor.WHITE + "왜곡된 공간의 어둠을 만든 후 빠르게 전방으로 이동한다.");
        } else {
            list.addFirst(ChatColor.WHITE + String.format("이동거리에 있던 대상에게 %.0f의 피해를 준다.", damage));
            list.addFirst(ChatColor.WHITE + "왜곡된 공간의 어둠을 만든 후 빠르게 전방으로 이동한다.");
        }
        return list;
    }
