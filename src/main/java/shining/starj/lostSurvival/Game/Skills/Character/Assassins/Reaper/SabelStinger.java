...
public class SabelStinger extends AbstractSkill {

    public SabelStinger() {
        super("샤핑 스팅거", Material.GRAY_DYE, Upgrade.COOL_DOWN, new Upgrade[]{Upgrade.DAMAGE, Upgrade.AREA, Upgrade.COOL_DOWN});
    }
    ...
    Vector[] vectors = new Vector[]{ //
            new Vector(0, 0, -0.8), new Vector(0.1, 0.1, -0.7), //
            new Vector(0.2, 0.2, -0.6), new Vector(0.3, 0.3, -0.5), //
            new Vector(0.4, 0.4, -0.4), new Vector(0.5, 0.5, -0.3), //
            new Vector(0.6, 0.6, -0.2), new Vector(0.7, 0.7, -0.1), //
            new Vector(0.8, 0.8, 0), //
            new Vector(0.8, 0.9, 0.1), new Vector(0.9, 1, 0.2), //
            new Vector(0.9, 1.1, 0.3), new Vector(1, 1.2, 0.4), //
            new Vector(1, 1.3, 0.5), new Vector(1, 1.4, 0.6), //
            new Vector(1, 1.5, 0.7), new Vector(1, 1.6, 0.8) //
    };
    ...
    public boolean Use(Player player, int level) {
        ...
        Bukkit.getScheduler().runTaskLater(Core.getCore(), () -> Attack(player, damage, -1), Math.min(10, getCoolDownTick(level) / 2));
        return true;
    }
    ...
    public List<String> getLore(int level) {
        ...
        list.addFirst(ChatColor.WHITE + String.format("적의 빈틈을 노려 날카롭게 찔러 %.0f의 피해를 주고, 잠시 후 더욱 빠르게 찔러 %.0f의 피해를 준다.", damage, damage));
        return list;
    }
