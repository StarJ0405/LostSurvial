...
public class BlackMist extends AbstractSkill {
    ...
    public List<String> getLore(int level, boolean upgrade) {
        ...
        if (level == 0) {
            list.addFirst(ChatColor.WHITE + "스킬 시전 중 받는 모든 피해가 감소하고, 주변에 계속 피해를 준다.");
            list.addFirst(ChatColor.WHITE + "빠르게 검은 그림자 속에 몸을 감춘 뒤, 관통 이동하며 적을 공격한다.");
        } else {
            list.addFirst(ChatColor.WHITE + String.format("스킬 시전 중 받는 모든 피해가 %.0f 감소하며, 주변에 계속 %.0f의 피해를 준다.", getReduce(level) * 100d, getDamage(level)));
            list.addFirst(ChatColor.WHITE + "빠르게 검은 그림자 속에 몸을 감춘 뒤, 관통 이동하며 적을 공격한다.");
        }
        return list;
    }
