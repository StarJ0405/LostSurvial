...
import shining.starj.lostSurvival.Entities.User.Shadow;
...
public class Blink extends AbstractSkill {
    ...
    public List<String> getLore(int level, boolean upgrade) {
        ...
        if (level == 0) {
            list.addFirst(ChatColor.WHITE + "1초마다 분신이 플레이어 방향으로 표창을 던져 피해를 준다.");
            list.addFirst(ChatColor.WHITE + "마우스 지점으로 그림자 분신을 소환한다.");
        } else {
